...
class QuranResourceService extends ChangeNotifier {
  static final QuranResourceService _instance = QuranResourceService._internal();
  ...
  String _sanitize(String s) => s.replaceAll(' ', '_').replaceAll('/', '_').replaceAll("'", "").replaceAll('"', '').replaceAll(':', '');

  String _getNameLang(String name, String lang, {bool useNative = true}) {
    final sName = _sanitize(name);
    final sLang = useNative ? _sanitize(lang) : (_langMap[lang] ?? _sanitize(lang));
    return '${sName}_$sLang';
  }

  /// Load a tafsir by its ID and Name_Language string
  Future<void> loadTafsir(String id, String nameLang) async {
    try {
      ...
      if (!exists) {
        ...
        final nameOnly = nameLang.contains('_') ? nameLang.split('_').first : nameLang;
    ...
    _selectedTafsirName = nameLang.contains('_') ? nameLang.split('_').first.replaceAll('_', ' ') : nameLang;
    _selectedTafsirLanguage = nameLang.contains('_') ? nameLang.split('_').last : 'English';
    notifyListeners();
    } catch (e) {
  ...
  Future<void> downloadResource(String category, String id, String name, String lang) async {
    final key = '${id}_$name';
  ...
  Future<bool> isDownloaded(String category, String name, String lang) async {
    ...
    final nameOnly = nameLangNative.contains('_') ? nameLangNative.split('_').first : nameLangNative;
    final paths = [
  ...
  Future<void> loadTranslation(String id, String nameLang) async {
    try {
      ...
      _selectedTranslationName = nameLang.contains('_') ? nameLang.split('_').first.replaceAll('_', ' ') : nameLang;
      _selectedTranslationLanguage = nameLang.contains('_') ? nameLang.split('_').last : 'English';
      notifyListeners();
    } catch (e) {
  ...
  Future<String?> getTafsirText(int surah, int ayah) async {
    ...
    String? groupKey = hasGroupKey ? result.first['group_ayah_key'] as String? : null;
    
    // 4. Resolve pointers if text is secondary (pointer resolution)
    if ((text == null || text.trim().isEmpty || (text.contains(':') && text.length < 10)) && groupKey != null && groupKey != key) {
      text = await _dbHelper.getText(_activeTafsirDbName!, groupKey, isTafsir: true);
    }
    ...
    if (hasGroupKey && groupKey != null) {
       ...
       if (allVerses.length > 1) {
         ...
         if (lang == 'bahasa indonesia' || lang == 'indonesian' || lang == 'indonesia') {
  ...
  Future<String?> getTranslationText(int surah, int ayah) async {
    if (_activeTranslationDbName == null) {
      debugPrint('getTranslationText: No active translation DB');
      return null;
    }
    
    String key = '$surah:$ayah';
    final db = await _dbHelper.getDatabase(_activeTranslationDbName!);
    debugPrint('getTranslationText: Querying $key in ${_activeTranslationDbName}');
    ...
    if (tableNames.contains('translation')) {
    ...
    debugPrint('getTranslationText: Found text for $key (length: ${text.length})');
