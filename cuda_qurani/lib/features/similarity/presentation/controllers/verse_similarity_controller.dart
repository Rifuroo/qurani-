...
class VerseSimilarityController extends ChangeNotifier {
  ...
  String? _verseText;
  String? _translation;
  List<SimilarVerse> _similarVerses = [];
  bool _isLoading = false;
  String? _errorMessage;

  VerseSimilarityController({
  ...
  List<SimilarVerse> get similarVerses => _similarVerses;
  ...
  Future<void> loadData() async {
    ...
    try {
      ...
      _translation = await _repository.getTranslationText(
        _surahId,
        _ayahNumber,
      );
      _similarVerses = await _repository.getSimilarVerses(
        _surahId,
        _ayahNumber,
      );
    } catch (e) {
      _errorMessage = 'Error loading similarities: $e';
    } finally {
      _isLoading = false;
      notifyListeners();
    }
  ...
  void _updateAyah(int newSurahId, int newAyahNumber) {
    _surahId = newSurahId;
    _ayahNumber = newAyahNumber;
    // Note: surahName update would be better handled via a call if surah changes
    loadData();
  }
